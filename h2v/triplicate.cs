namespace OpenH2.Scripts.Generatedscenarios.multi
{
    ...
    [OriginScenario("scenarios\\multi\\triplicate\\triplicate")]
    public partial class scnr_triplicate : ScenarioScriptBase
    {
#region Fields
        string data_mine_mission_segment;
        public scnr_triplicate(IScriptEngine scriptEngine)
        {
 ...
 == "normal";
        ...
        public async Task train_of_death()
        {
            Engine.object_destroy(eastbound_train.Entity);
            Engine.object_destroy(eastbound_death.Entity);
            Engine.object_destroy(westbound_train.Entity);
            Engine.object_destroy(westbound_death.Entity);
            await Engine.sleep((short)Engine.random_range(60, 360));
            if (0 == (short)Engine.random_range(0, 2))
            {
                Engine.object_create(eastbound_train);
                Engine.object_create(eastbound_death);
            }
            else
            {
                Engine.object_create(westbound_train);
                Engine.object_create(westbound_death);
            }
